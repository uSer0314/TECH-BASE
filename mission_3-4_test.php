<html>
	...
	<body>
				...
				<?php
					...
					$filename = "mission_3-4_test.txt";	//テキストファイルを作成して変数に代入
					...
					if(!empty($_POST["name"]) && !empty($_POST["comment"])) {		//入力された「名前」と「コメント」の両方が空でない場合
							...
							if(empty($_POST["editingNo"])) {		//新規投稿判定(editingNo：hiddenで受け取った編集中の番号)
									...
									$format = $number."<>".$name."<>".$comment."<>".$date;	//投稿フォーマットを作成し変数に代入
									fwrite($fp,$format."\n");		//ファイルに投稿を書き込む("\n"は改行を意味する)
									fclose($fp);	//ファイルを閉じる
									...
									foreach($file_array4 as $value3) {	//配列の要素数だけループ
										...
										if($element4[0] == $_POST["editingNo"]) {		//保持している編集番号と投稿番号が一致しているか判定
											...
											$format2 = $element4[0]."<>".$element4[1]."<>".$element4[2]."<>".$date;	//投稿フォーマットの作成($dateは編集日時)
											fwrite($fp,$format2."\n");	//ファイル編集した投稿を書き込み
			...
			<form action="mission_3-4_test.php" method="post">
				...
				<label>コメント：</label>
				<input type="text" name="comment" value="<?php if(!empty($editing_comment)) { echo $editing_comment; } ?>">	<!-- valueの中身は、編集したいコメントを表示するためのもの -->
			...
			<?php
				...
				$filename = "mission_3-4_test.txt";	//テキストファイルを作成
